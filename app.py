...
with tabs[0]:
    ...
    for p in st.session_state.pool_configs:
        ...
        if mode == "$":
            ...
            pct = c3.number_input(f"百分比", min_value=0, max_value=100, key=f"ap_{p['池名']}"); final_a = int(
                i_val * (pct / 100)); c3.write(f"折合 ${final_a}")
        total_alloc += final_a;
...
with tabs[1]:
    ...
    def get_item_category(item_desc, history_df, current_cats):
        ...
        if any(k in desc_up for k in
               ["餐", "麵", "飯", "飲", "茶", "水", "便當", "咖啡", "拿鐵", "蛋", "奶", "肉", "果", "食"]): return "飲食"
    ...
    with col_auto:
        ...
        with tab_email:
            ...
            st.link_button("👉 前往開啟 E-mail 消費明細通知",
                           "https://www.einvoice.nat.gov.tw/portal/btc/mobile/btc513w/main")
        with tab_csv:
            ...
            if uploaded_file is not None:
                if st.button("⚙️ 解析並匯入 CSV", key="csv_process_btn"):
                    try:
                        try:
                            csv_df = pd.read_csv(uploaded_file, encoding='big5')
                        except:
                        ...
                        if date_col and store_col and amt_col:
                            ...
                            for _, row in csv_df.iterrows():
                                ...
                                try:
                                    amt_val = float(str(row[amt_col]).replace(',', ''))
                                except:
                                    amt_val = 0
                                if len(date_val) == 8 and "/" not in date_val:
                                    date_str = f"{date_val[:4]}-{date_val[4:6]}-{date_val[6:]}"
                                elif "/" in date_val:
                                    date_str = date_val.replace("/", "-")
                                else:
                                    date_str = datetime.now().strftime('%Y-%m-%d')
                                memo = f"[載具] {store_val}"
                                existing = df[(df['日期'].astype(str) == date_str) & (
                                            df['備註'] == memo)] if not df.empty else pd.DataFrame()
                            ...
                            if new_rows:
                                ...
                                st.toast(f"✅ 成功匯入 {sync_count} 筆紀錄！", icon="📦");
                                time.sleep(1);
                                st.rerun()
                    except Exception as e:
                        st.error(f"❌ 解析失敗：{e}")
...
with tabs[2]:
        ...
        if st.button("⚡ 執行所有待處理扣款", key="execute_auto_btn"):
            ...
            for _, p in rec_df.iterrows():
                if f"[自動扣款] {p['項目名稱']}" not in deducted_m:
                    auto_rows.append(
                        [today_str, '支出', p['支出類別'], p['金額'], p['預算池'], f"[自動扣款] {p['項目名稱']}"])
    ...
    with st.container(border=True):
        ...
        if st.button("💾 儲存", key="save_auto_rule_btn"):
            if n and a > 0: rec_ws.append_row([n, a, p_auto, cat_auto, "1", "每月"]); st.toast(
                "✅ 儲存成功！"); st.rerun()
...
with tabs[3]:
    if df.empty:
        ...
        # 每月現金流流向直方圖
        monthly_delta = df.groupby('年月').apply(
            lambda x: x[x['類型'] == '收入']['金額'].sum() - x[x['類型'] == '支出']['金額'].sum()).reset_index()
        ...
        if mode == "📉 每日收入與花費":
            ...
            fig.update_layout(title=f"📊 {selected_month} 每日明細", barmode='relative',
                              xaxis=dict(type='date', tickformat='%d'))
            st.plotly_chart(fig, use_container_width=True)

        elif mode == "🎯 預算上限監控":
            config_df = pd.DataFrame(st.session_state.pool_configs)
            real_ex = df[df['類型'] == '支出'].groupby('帳戶')['金額'].sum().reset_index().rename(
                columns={'帳戶': '池名', '金額': '實際支出'})
            comp = pd.merge(config_df, real_ex, on='池名', how='left').fillna(0)
            comp['計算上限'] = comp.apply(
                lambda r: r['上限值'] if r['上限模式'] == "固定金額" else (r['上限值'] / 100 * total_in_all), axis=1)
            fig = go.Figure()
    ...
    with c1:
        ...
        for i, config in enumerate(st.session_state.pool_configs):
            with st.container(border=True):
                ...
                m_pool = st.selectbox("上限模式", ["固定金額", "百分比"],
                                      index=0 if config['上限模式'] == "固定金額" else 1, key=f"cm_{i}")
        ...
        if st.button("➕ 新增池", key="add_pool_btn"): st.session_state.pool_configs.append(
            {"池名": ap_new, "上限模式": "百分比", "上限值": 0}); st.toast("✅ 新增成功！"); st.rerun()
    with c2:
        st.subheader("🛠️ 支出類別")
        ...
        if st.button("➕ 新增項", key="add_cat_btn"): st.session_state.expense_cats.append(ne_cat); st.toast(
            "✅ 新增成功！"); st.rerun()
